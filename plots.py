...
axs[0,0].bar(*get_plot_data("SELECT strftime('%Y-%m', block_timestamp) AS dt, COUNT(*) AS c FROM text_results WHERE chain = 'btc' GROUP BY dt"), width=20)
axs[0,0].xaxis_date()
axs[0,0].set(xlabel="Time", ylabel="Transactions per month")
