...
axs[0, 0].set(xlabel="Time",ylabel="Amount")
...
""", False), '-')
axs[0, 1].set(xlabel="Text length in characters",ylabel="Amount")
axs[0, 1].set_xscale('log')

axs[1, 0].set_title("Frequency of Files Transactions")
axs[1, 0].plot_date(*get_plot_data("SELECT strftime('%Y-%m', block_timestamp) AS dt, COUNT(*) AS c FROM files_results GROUP BY dt"), '-')
axs[1, 0].set(xlabel="Time",ylabel="Amount")

axs[1, 1].set_title("Frequency of Files Transactions (Embedded Only)")
axs[1, 1].plot_date(*get_plot_data("SELECT strftime('%Y-%m', block_timestamp) AS dt, COUNT(*) AS c FROM files_results WHERE method = 'Embedded' GROUP BY dt"), '-')
axs[1, 1].set(xlabel="Time",ylabel="Amount")
