...
class FilesAnalysis:
	...
	def run(self):
		...
		if self.reset: cursor.execute("DROP TABLE IF EXISTS files_results")
		cursor.execute("CREATE TABLE IF NOT EXISTS files_results (hash TEXT, mime_type TEXT, method TEXT, to_contract BOOLEAN, data TEXT, block_timestamp DATETIME, deleted BOOLEAN DEFAULT 0)")
		self.conn.commit()
