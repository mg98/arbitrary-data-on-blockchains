...
class TextAnalysis(ABC):
	...
	def __init__(self, chain: str, limit: int = 0, reset: bool = False):
		"""
	...
	def insert(self, hash: str, data: str, value: int, block_timestamp: str, type: str, to_contract: bool = False):
		self.conn.execute("""
			INSERT INTO text_results (
				chain, hash, data, value, block_timestamp, type, to_contract
			) VALUES (?, ?, ?, ?, ?, ?, ?)
		""", (self.chain, hash, data, value, block_timestamp, type, to_contract))
		self.conn.commit()
	...
	def run(self):
		...
		if self.reset: 
			self.conn.execute("DELETE FROM text_results WHERE chain = ?", self.chain)
			self.conn.commit()
